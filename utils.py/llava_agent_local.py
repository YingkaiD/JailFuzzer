...
class LLaVAAgent(MultimodalConversableAgent):
    ...
    def _image_reply(self, messages=None, sender=None, config=None):
        ...
        print(colored(self.llm_config, 'red'))
...
def _llava_call_binary_with_config(
    ...
    if config["base_url"].find("0.0.0.0") >= 0 or config["base_url"].find("localhost") >= 0 or config["base_url"].find("10.102.32.161") >= 0 or config["base_url"].find("10.102.32.7") >= 0:
        llava_mode = "local"
    else:
        llava_mode = "remote"
