...
from filters.text_based_filter import text_check
...
class ModelWorker():
    def __init__(self):
        self.base = None
        self.refiner = None
        self.text_filter_model = None
        self.text_filter_tokenzier = None
        self.g_cuda = torch.Generator(device)
        self.g_cuda.manual_seed(0)
        self.check = text_check
    ...
    def load_pretrained_model(self):
        ...
        self.text_filter_tokenizer = AutoTokenizer.from_pretrained("/data/yingkai/autosp/NSFW_text_classifier")
        self.text_filter_model = AutoModelForSequenceClassification.from_pretrained("/data/yingkai/autosp/NSFW_text_classifier")
        self.text_classifier = pipeline("sentiment-analysis", tokenizer=self.text_filter_tokenizer, model=self.text_filter_model)

    def generate_image(self, prompt):
        
        nsfw = self.check([prompt], self.text_classifier)
...
if __name__ == '__main__':
    ...
    uvicorn.run(app, host="0.0.0.0", port=31005, log_level="info")
